const Sidebar = ({ activeTab, setActiveTab, darkMode, toggleDarkMode }) => {
  ...
  return (
    <aside className="flex h-full flex-col rounded-none border-r border-zinc-800 bg-zinc-950/95 p-4 shadow-xl shadow-black/25 transition-all duration-300 backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/95 dark:shadow-black/40">
      {/* Logo / Marca */}
      ...
      <nav className="flex-1 space-y-1">
        {menuItems.map((item) => (
          <button
            ...
            className={`group w-full flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold transition-all duration-200 ${
              activeTab === item.key
